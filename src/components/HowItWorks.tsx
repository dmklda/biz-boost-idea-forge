        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, index) => (
            <div key={index} className="relative">
              <div className="bg-white p-8 rounded-xl shadow-sm h-full flex flex-col animate-fade-in">
                <div className="text-5xl font-bold text-brand-purple/10 mb-4 font-poppins">
                  {step.number}
                </div>
                <h3 className="text-xl font-semibold mb-3 font-poppins">{step.title}</h3>
                <p className="text-gray-600 flex-grow font-inter">{step.description}</p>
