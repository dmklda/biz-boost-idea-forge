...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: existingInsights } = await supabase.from('daily_insights').select('*').eq('user_id', user.id).eq('generated_date', today).single();
    if (existingInsights) {
    ...
    const { data: ideas } = await supabase.from('ideas').select('*').eq('user_id', user.id).order('created_at', {
      ascending: false
    });
    const { data: analyses } = await supabase.from('idea_analyses').select('*').eq('user_id', user.id).order('created_at', {
      ascending: false
    });
    const { data: recentTransactions } = await supabase.from('credit_transactions').select('*').eq('user_id', user.id).order('created_at', {
      ascending: false
    }).limit(10);
    ...
    const userData = {
      ...
      recentActivity: {
        ideasThisWeek: ideas?.filter((idea)=>new Date(idea.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length || 0,
        analysesThisWeek: analyses?.filter((analysis)=>new Date(analysis.created_at) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length || 0,
      ...
      performance: {
        ...
        improvementTrend: analyses?.length >= 2 ? analyses.slice(0, 3).reduce((sum, a)=>sum + a.score, 0) / 3 - analyses.slice(-3).reduce((sum, a)=>sum + a.score, 0) / 3 : 0
      },
      engagement: {
        creditsUsed: recentTransactions?.filter((t)=>t.amount < 0).length || 0,
        favoriteIdeasCount: 0,
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Você é um especialista em análise de dados de empreendedorismo que gera insights personalizados para ajudar empreendedores a melhorar seus resultados. Sempre retorne exatamente 3 insights únicos e diferentes.'
          },
          {
            role: 'user',
            content: prompt
    ...
    const { data: savedInsights, error: saveError } = await supabase.from('daily_insights').insert({
      user_id: user.id,
