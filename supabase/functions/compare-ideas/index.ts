  ...
  try {
    ...
    try {
      ...
      console.log('Request body parsed:', {
        ideas: requestBody.ideas?.length
      });
    } catch (parseError) {
    ...
    if (!Array.isArray(ideas) || ideas.length < 2 || ideas.length > 3) {
      ...
      return new Response(JSON.stringify({
        error: 'Deve fornecer entre 2 e 3 ideias para comparação'
      }), {
        status: 400,
        headers: {
          ...corsHeaders,
...
IMPORTANTE: Responda APENAS com o JSON, sem texto adicional antes ou depois.`;
    ...
    const { data: comparisonData, error: comparisonError } = await supabaseClient.from('idea_comparisons').insert({
      user_id: user.id,
      idea_ids: ideaIds,
...
function createFallbackInsights(ideasData, missingFields = [], partialInsights = {}) {
