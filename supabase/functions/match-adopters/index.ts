...
serve(async (req) => {
  ...
  try {
    ...
    const { data: adopters, error: adoptersError } = await supabaseClient
      .from('early_adopters')
      .select('*')
    ...
    if (!adopters || adopters.length === 0) {
      return new Response(
        JSON.stringify({ matches: [], message: 'No available adopters found' }),
...
function calculateSimpleMatch(adopter: EarlyAdopter, criteria: MatchingCriteria): number {
  ...
  // Expertise match
  if (adopter.expertise_areas?.some(area => 
    area.toLowerCase().includes(criteria.category.toLowerCase())
  )) {
    score += 15;
  }
