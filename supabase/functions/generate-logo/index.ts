...
serve(async (req)=>{
  ...
  try {
    ...
    const contentType = logoOutputFormat === 'png' ? 'image/png' : logoOutputFormat === 'jpeg' ? 'image/jpeg' : 'image/webp';
    const { data: uploadData, error: uploadError } = await supabase.storage.from('generated-content').upload(fileName, imageData, {
